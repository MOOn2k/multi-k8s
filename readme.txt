...
SKAFFOLD: разработка без необходимости пушиться на докерхаб
---
Позволяет избежать необходимости пуша изменений на докер хаб для получения их в кластере minikube путем отслеживания
изменений в папке нужного докер имейджа и применения их в кластере:
1. через билд (дольше, но гарантированно работает)
2. через подмену измененных файлов (значительно быстрее, но не будет, например, работать без использования того же
ноудмона для реакта и схожих решений при их необходимости)
Устанавливается так
> brew install skaffold
На проекте применяется путем создания файла объекта Config в корне проекта.
Чтобы запустить выполняем следующее
> skaffold dev
...
Для каждого компонента приложения написан рабочий вариант Dockerfile'а, конфига, описывающего Docker образ, из которого в последствии и запускался бы соответствующий контейнер с компонентом.

Получен опыт ручной сборки и запуска каждого из компонентов с необходимым набором параметров, описывающих глобальные переменные,  волъюмсы и тд.

Для упрощения процесса разработки применен Docker Compose, позволивший через свой конфиг объединить весь процесс сборки Docker образов и запуска контейнеров из них в пару команд.

избавиться от необходимости запускать  каждый раз в ручную собирать и запускать каждый Docker Image по отдельности

Для упрощения запуска приложения в среде разработки, все его компоненты объединены средствами Docker Compose, убраны.

Для каждого из компонентов написан боевой Dockerfile, ис.


Получен опыт применения Docker для разделения приложения на сервисы.


Собрано тестовое Node.js приложение "Калькулятор рядов Фибаначи". Приложение состоит из сервисов бекенда (Express), фронтенда (React), воркера и использует в качестве БД PostgreSQL и Redis.

Получен навык создания приложения, состоящего из нескольких сервисов, общающихся по http
